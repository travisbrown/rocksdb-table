use super::{IndexedTable, Table};
use rocksdb::{DBAccess, DBIteratorWithThreadMode};
use std::borrow::Cow;
use std::marker::PhantomData;

pub struct TableIterator<'a, D: DBAccess, T> {
    underlying: DBIteratorWithThreadMode<'a, D>,
    _table: PhantomData<T>,
}

impl<'a, D: DBAccess, T> TableIterator<'a, D, T> {
...
impl<'a, D: DBAccess, T: Table> Iterator for TableIterator<'a, D, T> {
...
pub struct SelectedValueTableIterator<'a, D: DBAccess, T, P> {
    underlying: DBIteratorWithThreadMode<'a, D>,
    pred: P,
    _table: PhantomData<T>,
}
...
impl<'a, D: DBAccess, T: Table, P: Fn(&T::Key) -> bool> Iterator
    for SelectedValueTableIterator<'a, D, T, P>
{
